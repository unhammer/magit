...
Magit depends on the ~dash~, ~ghub~, ~graphql~, ~lv~, ~magit-popup~, ~transient~, ~treepy~ and
...
If you haven't installed ~dash~, ~ghub~, ~graphql~, ~lv~, ~magit-popup~, ~transient~, ~treepy~
and ~with-editor~ from Melpa or at ~/path/to/magit/../<package>~, then you
...
Note that ~lv.el~ is part of the ~hydra~ repository.
